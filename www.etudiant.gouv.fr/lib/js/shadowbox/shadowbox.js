...
(function(){
    ...
    K = {
        ...
        reinitPictoClose: function() {
            el = document.getElementById('sb-nav-close');

            if(el) {
                    var saveBackground = el.style.backgroundImage;
                    saveBackground = saveBackground.replace('url(', '');
                    saveBackground = saveBackground.replace(')', '');
                    el.style.backgroundImage = 'none';
                    el.style.filter = 'progid:DXImageTransform.Microsoft.AlphaImageLoader(enabled=true,src=' +saveBackground + ',sizingMethod=scale);';
        ...
        dynamicResize: function(w, h){
            if(S.content){
                S.content.width = w;
                S.content.height = h;
                var d = setDimensions(S.content.height, S.content.width, S.content.resizable); 
                this.resizeContent(h,w,0,0,function(){;}); //needs callback function to animate
            }
        }
